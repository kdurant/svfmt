...
begin
    ...
    begin
        ...
        case (state)
            ST_RDID_CMD:    if (axis_m_tvalid && axis_flash_wr.tready) byte_cnt <= spi_flash.len;
            ST_READ_ADDR_B0: if (axis_m_tvalid && axis_flash_wr.tready) byte_cnt <= spi_flash.len;
            ST_PP_ADDR_B0:  if (axis_m_tvalid && axis_flash_wr.tready) byte_cnt <= spi_flash.len;
            ST_RDID_DATA:   if (axis_m_tvalid && axis_flash_wr.tready && byte_cnt > 1) byte_cnt <= byte_cnt - 1;
            ST_READ_DATA:   if (axis_m_tvalid && axis_flash_wr.tready && byte_cnt > 1) byte_cnt <= byte_cnt - 1;
            ST_PP_DATA_TX:  if (axis_m_tvalid && axis_flash_wr.tready && byte_cnt > 1) byte_cnt <= byte_cnt - 1;
        endcase
...
begin
    ...
    case(state)
        ...
        // ---- RDID (0x9F) ----
        ST_RDID_CMD  : if(axis_handshake) state_nxt = ST_RDID_DATA;
        ST_RDID_DATA : if(axis_handshake)
    begin
        if(byte_cnt   <= 1)
        ...
        // ---- Busy Poll / RDSR ----
        ST_POLL_CMD  : if(axis_handshake) state_nxt = ST_POLL_RX;
        ST_POLL_RX   : if(axis_handshake) state_nxt = ST_POLL_WAIT;
        ST_POLL_WAIT : if(poll_done)
    begin
        if(is_user_rdsr)
        ...
        // ---- SE (0x20) ----
        ST_SE_CMD     : if(axis_handshake) state_nxt = ST_SE_ADDR_B2;
        ST_SE_ADDR_B2 : if(axis_handshake) state_nxt = ST_SE_ADDR_B1;
        ST_SE_ADDR_B1 : if(axis_handshake) state_nxt = ST_SE_ADDR_B0;
        ST_SE_ADDR_B0 : if(axis_handshake) state_nxt = ST_SE_DONE;
